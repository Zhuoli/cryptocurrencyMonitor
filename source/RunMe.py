...
def Monitor(email):
    ...
    # Query current goods price from APIs
    for goodsName in goodsNames:
        namePricePairs[goodsName] = digitCoinApi.RetrievePriceForGoods(goodsName)


    # Retrieve history coin price from disk csv file
    CSVReader = DataKeeper(goodsNames, DATA_PATH)

    coinsHistoryArray, dateArray = CSVReader.InitCoinsHistory(num, namePricePairs)

...
ALLCURRENCY = 'http://coinmarketcap-nexuist.rhcloud.com/api/all'
...
def main():
    ...
    try:
        ...
        print "Email authenticate failed, quite application: ", e
        Log("Email authenticate failed, quite application: " + str(e))
        sys.exit(-1)
        
    print "Email authenticate succeed"
...
def Monitor(email):
    num = MONITOR_WINDOW / SLEEP_SECONDS;
    
    #Currency Object Init
    coins = map(lambda coin : CurrencyQuery(coin), COINS);
    coinnames = map(lambda coin : coin.name, coins)
    map(lambda coin : coin.Refresh(), coins)
    prices = map(lambda coin : coin.GetPrice(), coins) 
...
class CurrencyQuery:
    ...
    def Refresh(self):
        ...
        self.resp, self.content = httplib2.Http().request(self.url)
